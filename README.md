...
│   ├── data/                     # 1회성 데이터 준비 파이프라인 (얇은 CLI, 로직은 src/preprocessing)
│   │   ├── download_videos.py        #   YouTube 영상 다운로드 (yt-dlp 래퍼, URL 은 YouCookII JSON 에서 추출)
│   │   ├── extract_frames.py         #   2 FPS 프레임 추출 (선별 300개, src/preprocessing 공용 함수)
│   │   ├── generate_annotation_labels.py  #   프로토타입 자동 분류 + 검수 분기 (로직: annotation_labeling.py)
│   │   └── generate_frame_labels.py  #   segment 통합 → 프레임 라벨 + Soft Boundary (로직: frame_labeling.py)
│   ├── train.py                  # 학습 엔트리포인트 (ML 진입점, 반복 실행)
...
[scripts/data/download_videos.py](scripts/data/download_videos.py) 는 [data/external/YouCookII/annotations/youcookii_annotations_trainval.json](data/external/YouCookII/annotations/youcookii_annotations_trainval.json) 의 각 항목 `video_url` 필드에서 다운로드 URL 을 직접 추출해 yt-dlp 로 받아 [data/raw/](data/raw/) 에 저장합니다.
