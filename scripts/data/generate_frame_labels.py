...
[입력]
  data/processed/action_annotations.csv   (자동, good/weak 가중치 보유)
  data/processed/reviewed_annotations.csv (사람 확정, 가중치 1.0 일괄 부여)
  data/frames/<video_id>/timestamps.json (영상별 프레임 timestamp)

[출력]
  data/processed/frame_labels/<video_id>.csv (영상당 1개, 학습 직접 입력)
    컬럼: frame_index, frame_name, timestamp_sec,
          label, source, segment_id, sample_weight, is_boundary
  data/processed/frame_labels/_manifest.json
...
ACTS_PATH = ROOT / "data" / "processed" / "action_annotations.csv"
REVIEWED_PATH = ROOT / "data" / "processed" / "reviewed_annotations.csv"
...
OUTPUT_ROOT = ROOT / "data" / "processed" / "frame_labels"
